...
type TwiddleTCGo[C <: NonEmptyTuple, A <: Tuple] = 
  (C, A) match
    ...
    case (TypedColumn[?, b1, ?, ?] *: TypedColumn[?, b2, ?, ?] *: EmptyTuple, EmptyTuple) => b1 ~ b2
    case (TypedColumn[?, b1, ?, ?] *: TypedColumn[?, b2, ?, ?] *: tail,       EmptyTuple) => TwiddleTCGo[tail, b1 ~ b2]
...
type TwiddleTCN[C <: NonEmptyTuple] <: Product =
  C match
    ...
    case TypedColumn[?, c1, ?, ?] *: TypedColumn[?, c2, ?, ?] *: t => TwiddleTCNGo[t, c1 ~ c2]
type TwiddleTCNGo[C <: Tuple, A <: Tuple] <: Tuple =
  (C, A) match
    case (EmptyTuple, acc) => A
    case (TypedColumn[?, c, ?, ?] *: t, acc) => TwiddleTCNGo[t, acc ~ c]

type NonRequiredConstraints = (TypedColumn.Constraint.Nullable.type, TypedColumn.Constraint.Default.type)
...
type Required[C <: NonEmptyTuple] <: Tuple =
  C match
    case TypedColumn[n, ?, ?, c] *: t         => RequiredGo[t, IfInM[c, NonRequiredConstraints, EmptyTuple, n *: EmptyTuple]]
type RequiredGo[C <: Tuple, A <: Tuple] <: Tuple = 
  C match
    case EmptyTuple                           => A
    case TypedColumn[n, ?, ?, c] *: t         => RequiredGo[t, IfInM[c, NonRequiredConstraints, A, Append[A, n]]]
...
/** Match type to transform `(a, b, c, d)` of `TypedColumn.In` into `(((a, b), c), d)` (or `a ~ b ~ c ~ d`) */
type TwiddleIn[I <: NonEmptyTuple] =
  I match
    case TypedColumn.In[?, ?, b]  *: EmptyTuple => Tuple1[b]
    case TypedColumn.In[?, ?, b1] *: TypedColumn.In[?, ?, b2] *: t => TwiddleInGo[t, b2 ~ b1]
