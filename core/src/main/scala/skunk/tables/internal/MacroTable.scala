...
/**
  * MacroTable is a class containing all information necessary for `Table` synthezis.
  * It can be of two phases, depending on how much information we have about type `A`
  */
sealed trait MacroTable[Q <: Quotes & Singleton, A]:
  ...
  /**
   * Get ordered tuple of all columns as `TypedColumn`
   * Note: it creates only values here, types for `Columns` created there within refinement
   */
...
object MacroTable:
  ...
  class InitPhase[Q <: Quotes & Singleton, A](val quotes: Q,
                                              val tpe: Type[A],
                                              val columnMap: NonEmptyList[(String, (quotes.reflect.TypeRepr, Expr[IsColumn[?]]))]) extends MacroTable[Q, A]:
    import quotes.reflect.*

    def next(constraints: List[(String, quotes.reflect.TypeRepr, quotes.reflect.TypeRepr)], tableName: String): FinalPhase[Q, A] =
  ...
  class FinalPhase[Q <: Quotes & Singleton, A](val quotes: Q,
                                               val tpe: Type[A],
                                               ...
                                               val tableName: String) extends MacroTable[Q, A]:
    import quotes.reflect.*

    def getConstraint(label: String): TypeRepr =
      constraints.find((name, _, _) => name == label) match
        case Some((_, _, tpr)) => tpr
        case None if constraints.isEmpty => TypeRepr.of[EmptyTuple]     // It means the method being called on first stage
        case None => report.errorAndAbort(s"Column with name $label was not found. Check consistency of Table building. Known columns")
      ...
      columnMap.toList.map { case (n, (t, p)) =>
        ...
        (nameExpr.asTerm.tpe.asType, t.asType, tableNameType, constraint.asType) match
          case ('[name], '[tpe], '[tableName], '[constr]) =>
            val name = nameExpr.asExprOf[name & SSingleton]
            '{ new TypedColumn[name & SSingleton, tpe, tableName, constr & Tuple](${ name }, ${ p.asExprOf[IsColumn[tpe]] }) }
      }
  ...
  def build[T: Type](using quotes: Quotes): MacroTable.InitPhase[quotes.type, T] =
    ...
    val columnMap = NonEmptyList.fromList(flattenProduct[T](Nil)(mirror.elems).map((path, tpe) => snakeCase(path.last) -> tpe)) match
      case Some(nel) => nel
      case None => report.errorAndAbort("Could not derive columns. A Columns must contain at least one Column")
  ...
  def flattenProduct[T](using quotes: Quotes)(root: List[String])(ls: List[MirrorElem[quotes.type, T, ?]]): List[(NonEmptyList[String], (quotes.reflect.TypeRepr, Expr[IsColumn[?]]))] =
    import quotes.reflect.*
          ...
          Expr.summon[IsColumn[t]] match
            ...
            case None =>
              MacroMirror.summon[t] match
                ...
                case _ =>
                  quotes.reflect.report.errorAndAbort(s"Couldn't synthesize IsColumn instance for ${TypeRepr.of[t].show}, which also happens to be not a Product type")
    }
