...
/** `Columns` is a "selectable" trait, which means the members of it are created dynamically at
  ...
  */
trait InsertSelect[F] extends Selectable:
  ...
  override def toString: String =
    s"InsertSelect(${get.map(c => s"${c.n}: ${c.primitive.codec.types.mkString(",")}").mkString(", ")})"
    ...
    (typedColumns.asTerm.tpe.asType, refinement.asType) match
      case ('[typedColumns], '[refinement]) =>
        '{
          ...
          ).asInstanceOf[InsertSelect[?] { type TypedColumns = typedColumns } & refinement]
        }
