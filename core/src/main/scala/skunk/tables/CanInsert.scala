...
/**
 * `CanInsert` is an evidence that a value of type `A` can be inserted into `Table[T]`
 * In nutshell it's a mapping from members of `A` into columns of `Table[T]`.
 * `A` can have more members than `Table[T]` columns, but every non-nullable,
 * non-default member of `Table[T]` must have a counterpart in `A`
 * 
 * It exists to handle different scenarios, where table has `DEFAULT` or `NULL` columns
 * and we need to reliably map class members to their respective columns.
 * Each table can have multiple user-defined instances of `CanInsert` for different classes
 */
trait CanInsert[A, T]:


  /** Tuple (a twiddled list to be precise) of elements `A` (not necessarily isomorphic to `A` or `T`) */
  type Twiddled
  
  /** 
   * List of columns of `A` that this instance can fulfill. Used in `INSERT` statement
   * Order of columns does NOT represent order of members of `A`, nor columns of `T`
   * It matches to order of `Twiddled`, which is specified by `via` contructor function
   */
  ...
  /**
   * Every time we insert `a` into a table we need to transform it into
   * a twiddle list, ordered for `columns`
   */
...
object CanInsert:
  ...
  inline transparent def apply[A]: CanInsertPartialInit[A] = new CanInsertPartialInit[A] { }

  trait CanInsertPartialInit[A]:
    inline transparent def into[T <: Product, S, C <: NonEmptyTuple](table: Table[T] { type Select = S; type TypedColumns = C }): CanInsertPartialFinal[A, T, S, C] =
  ...
  trait CanInsertPartialFinal[A, T <: Product, S, C <: NonEmptyTuple]:
    ...
    /**
     * Last stage of `CanInsert` generation
     * Here we map members of `A` into columns of `Table[T]`
     *
     * @warn for single-element mapping you should wrap it into `Tuple1.apply` 
     *       and *not* use `x *: EmptyTuple` constructor
     */
    inline transparent def via[I <: NonEmptyTuple](map: S => I)(using AllRequired[C, I]): CanInsert[A, T] =
      new CanInsert[A, T]:
        ...
        override def toString: String = s"""CanInsert(${tableOfT.name}, [${columns.mkString(", ")}])"""
  ...
  private inline def getCodecGo[T <: Tuple, A](a: A): Any =
    inline erasedValue[T] match
      case _: EmptyTuple => a
      case _: (TypedColumn.In[?, ?, t] *: ts) => getCodecGo[ts, Codec[A ~ t]](a.asInstanceOf[Codec[A]] ~ summonInline[IsColumn[t]].codec)
      
  /**
   * Convert an object of `A` into a twiddle list, described by `C`
   * Where `C` is a tuple of `TypedColumn.In`, containing functions for destructuring `A`
   */
  transparent inline def buildTwiddled[C <: NonEmptyTuple, A](c: C, a: A): Any =
    ${ buildTwiddledImpl[C, A]('c, 'a) }
  private def buildTwiddledImpl[C <: NonEmptyTuple : Type, A: Type](using Quotes)(cExpr: Expr[C], aExpr: Expr[A]): Expr[Any] =
