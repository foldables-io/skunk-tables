...
class CanInsertSuite extends FunSuite:
  test(
    "CanInsert can be generated for simple case class with all non-Option fields"
  ) {
  ...
  test(
    "CanInsert cannot be generated if at least one required field is missing in mapping"
  ) {
    case class Person(id: Long, firstName: String, age: Int)
    case class PersonWithId(id: Int, firstName: String, age: Int)
    val table = Table.of[Person].withName("persons").build
    ...
    CanInsert[PersonWithId]
      ...
      .via(columns =>
        (
          columns.id.from[PersonWithId](_.id),
          columns.first_name.from[PersonWithId](
            _.firstName
          ), // We can drop this. For some reasons compileErrors doesn't work
          columns.age.from[PersonWithId](_.age),
        )
      )
  }
  ...
  test(
    "CanInsert can be generated if an optional field is missing in mapping"
  ) {
